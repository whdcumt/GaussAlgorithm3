...
//均值模糊函数声明
void boxBlur_2(IplImage *scl,IplImage *tcl,int w,int h,int r); 
int _tmain(int argc, _TCHAR* argv[])
{
	...
	gaussBlur_3(rImg, rImgBlur,rImgBlur->width ,rImgBlur->height,10);
	gaussBlur_3(gImg, gImgBlur,gImgBlur->width ,gImgBlur->height,10);
	gaussBlur_3(bImg, bImgBlur,bImgBlur->width ,bImgBlur->height,10);
        ...
        for(int j=0; j<w; j++) 
		{
			...
			for(int iy=i-r; iy<i+r+1; iy++)
                for(int ix=j-r; ix<j+r+1; ix++) 
				{
                     x = MIN(w-1, MAX(0, ix));
                     y = MIN(h-1, MAX(0, iy));
                    val += cvGet2D(scl,y,x).val[0];
                }
			s.val[0]=val/((r+r+1)*(r+r+1));
            cvSet2D(tcl,i,j, s); 
        }
}
